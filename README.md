...
**JOSEONin**은 선박 건조 현장의 용접 품질 관리를 자동화하기 위해 개발된 **AI 기반 비전 검사 솔루션**입니다.

YOLO(You Only Look Once) 객체 탐지 모델을 활용하여 용접 부위를 실시간으로 분석하고, **양품, 불량, 블루홀** 등 3가지 상태로 정밀 분류하여 육안 검사의 한계를 극복하고 안전성을 높이는 것을 목표로 합니다.
...
## 🚩 프로젝트 개요 (Overview)
### 배경 및 문제 인식
* **숙련 인력 부족:** 조선업계의 숙련 용접 인력 고령화 및 청년층 유입 저조로 인한 인력난이 심화되고 있습니다.
* **안전 및 품질 이슈:** LNG 운반선 등에서 발생하는 미세 용접 결함은 누설 및 폭발 사고로 직결될 수 있으나, 기존 육안 검사로는 미세 균열 탐지에 한계가 존재합니다.

### 솔루션 목표
* **자동화 검사:** 카메라와 AI를 활용하여 용접 부위를 자동으로 검수하고 데이터를 수집합니다.
* **효율성 증대:** 검사 시간을 단축하고 재작업 비용을 절감하여 조선 공정의 생산성을 향상시킵니다.

---

## 🔍 주요 기능 (Key Features)
* **실시간 결함 탐지:** 카메라로 촬영된 영상을 분석하여 용접 부위의 상태를 즉각적으로 판독합니다.
* **3단계 품질 분류:** 용접 상태를 아래 3가지 유형으로 식별합니다.
    * **Good (양품):** 정상적인 용접 상태
    * **Bad (불량):** 용접 불량 상태
    * **Bluehole (블루홀):** 기공 등 특정 결함(블루홀)이 발생한 상태
* **데이터 파이프라인 최적화:** 현장 데이터를 수집하고 학습 가능한 형태로 변환하는 자동화 파이프라인을 구축했습니다.
